...
{
	gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, this.id);
	gl.bufferData(gl.ELEMENT_ARRAY_BUFFER, nbv*this.nb_floats, gl.STATIC_DRAW);
	gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, null);
},
...
function VAO()
{
	...
	return Object.assign(Object.create(VAO_ops),{id,length});
}
...
let ShaderProgram_ops=
{
	...
	search_attribute_loc: function()
	{
		this.locations.forEach( (v,k) => 
		{
			if (v<0) 
			{
				...
				Object.defineProperty(this.in, k, { value:loc });
			}
			else
			{
				Object.defineProperty(this.in, k, { value:v });
			}
		});
		Object.seal(this.in);
	},
	...
	set_uniform_func: function(uname,f)
	{
		Object.defineProperty(this.uniform,	uname, { set:f });
	},
...
function ShaderEmptyProgram()
{
	...
	let prg = null;
	let o = Object.assign(Object.create(ShaderProgram_ops),{locations, uniform:{}, in:{}, prg});
	return o;
}
...
function ShaderProgram(vert, frag, name, fixed_attr=[])
{
	let locations = new Map();
	let prg = null;
	let o = Object.assign(Object.create(ShaderProgram_ops),{v_src:vert,f_src:frag,locations, uniform:{}, in:{}, prg, sh_name:name});
...
function ShaderTransformFeedbackProgram(vert, name, outs, fixed_attr=[])
{
	let locations = new Map();
	let prg = null;
	let o = Object.assign(Object.create(ShaderProgram_ops),{v_src:vert,locations, uniform:{}, in:{}, prg, sh_name:name});
...
let Texture2d_ops =
{
	init: function(iformat)
	{
		this.iformat = iformat;
		this.width  = 0;
		this.height = 0;
		gl.bindTexture(gl.TEXTURE_2D, this.id);
		gl.texImage2D(gl.TEXTURE_2D, 0, iformat, 0, 0, 0, gl.RGB, gl.UNSIGNED_BYTE, null);
...
gen_vao: function (p=true, n=true, t=true)
{
	let nbv = 0;
	if (p && !this.vbo_p)
	{
	...
	return VAO([POSITION_ATTRIB, p?this.vbo_p:null], [NORMAL_ATTRIB, n?this.vbo_n:null], [TEXCOORD_ATTRIB, t?this.vbo_t:null]);
},
...
renderer: function (p=true, n=true, t=true)
{
	...
	if (this.ebo_tri === undefined)
	{
		this.ebo_tri = EBO(this.tris);
		this.ebo_line = EBO(this.lines);
	}

	return Object.assign(Object.create(MeshRenderer_ops),{vao, ebo_tri:this.ebo_tri, ebo_line:this.ebo_line, nbv});
}
};
...
let MeshRenderer_ops = {
	draw : function(prim, vao=null)
	{
		...
		case gl.TRIANGLES:
			this.ebo_tri.bind();
			gl.drawElements(gl.TRIANGLES, this.ebo_tri.length, gl.UNSIGNED_INT, 0);
			unbind_ebo();
			break;
		case gl.LINES:
			this.ebo_line.bind();
